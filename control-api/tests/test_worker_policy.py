...
def test_route_instance_failure_generalizes_beyond_the_flagship_cell() -> None:
    failed = {("auth", "inst-a")}
    decision = classify(evidence(failing=failed), probes(failing=failed), [])
    assert decision.final_class == "ROUTE_INSTANCE_FAILURE"
    assert decision.actionable is True
    assert decision.route == "auth"
    assert decision.instance == "inst-a"
    assert {item["class"] for item in decision.competing} == {"INSTANCE_DOWN", "SHARED_ROUTE_FAILURE"}


def test_instance_down_generalizes_to_any_physical_instance() -> None:
    failed = {(route, "inst-c") for route in ROUTES}
    decision = classify(evidence(failing=failed), probes(failing=failed), [])
    assert decision.final_class == "INSTANCE_DOWN"
    assert decision.instance == "inst-c"


def test_shared_route_failure_generalizes_beyond_checkout() -> None:
    failed = {("auth", instance) for instance in INSTANCES}
    decision = classify(evidence(failing=failed), probes(failing=failed), [])
    assert decision.final_class == "SHARED_ROUTE_FAILURE"
    assert decision.route == "auth"


def test_statistical_support_is_advisory_and_never_changes_actionability() -> None:
    shadow = {"auth/inst-a": {"recommended_weight": 20, "flagged_since": "t0", "flagged_seconds": 12.0}}
    decision = classify(evidence(), probes(), [], statistical_support=shadow)
    assert decision.final_class == "HEALTHY"
    assert decision.actionable is False
    assert decision.support["shadow_statistical_signal"] == shadow

    failed = {("checkout", "inst-b")}
    decision = classify(evidence(failing=failed), probes(failing=failed), [], statistical_support=shadow)
    assert decision.final_class == "ROUTE_INSTANCE_FAILURE"
    assert decision.support["shadow_statistical_signal"] == shadow
